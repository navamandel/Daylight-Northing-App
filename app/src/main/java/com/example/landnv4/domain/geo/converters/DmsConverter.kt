...
object DmsConverter {
    ...
    fun validate(p: Dms): List<String> {
        val errors = mutableListOf<String>()

        fun checkLat() {
            if (p.latHem !in charArrayOf('N','S')) errors += "Latitude hemisphere must be N or S"
            if (p.latDeg !in 0..90) errors += "Latitude degrees must be 0..90"
            if (p.latMin !in 0..59) errors += "Latitude minutes must be 0..59"
            if (!p.latSec.isFiniteReal() || p.latSec !in 0.0..<60.0) errors += "Latitude seconds must be 0..60"
            if (p.latDeg == 90 && (p.latMin != 0 || p.latSec != 0.0)) errors += "Latitude 90° must have 0'0\""
        }

        fun checkLon() {
            if (p.lonHem !in charArrayOf('E','W')) errors += "Longitude hemisphere must be E or W"
            if (p.lonDeg !in 0..180) errors += "Longitude degrees must be 0..180"
            if (p.lonMin !in 0..59) errors += "Longitude minutes must be 0..59"
            if (!p.lonSec.isFiniteReal() || p.lonSec !in 0.0..<60.0) errors += "Longitude seconds must be 0..60"
            if (p.lonDeg == 180 && (p.lonMin != 0 || p.lonSec != 0.0)) errors += "Longitude 180° must have 0'0\""
        }
    ...
    fun parse(input: String): Dms {
        ...
        // val latDe = GeoCore.dmsToDecimal(parts[0].toInt(), parts[1].toInt(), parts[2].toDouble(), parts[3][0].toChar())
        //val lonDe = GeoCore.dmsToDecimal(parts[4].toInt(), parts[5].toInt(), parts[6].toDouble(), parts[7][0].toChar())
